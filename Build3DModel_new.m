function [output_vertices, foreobj_3D]= Build3DModel_new(points, img, foreobj_2D)
...
% Then tansform them to 3D coordinates.
    ...
    %%% Min&Max Boundaries in screen coordinate
    %for i = 2:13
    %    if output_vertices_in_screen(1, i) < screen_min_x
    %    screen_min_x = output_vertices_in_screen(1, i);
    %    elseif output_vertices_in_screen(1, i) > screen_max_x
    %       screen_max_x = output_vertices_in_screen(1, i);
    %    end

    %    if output_vertices_in_screen(2, i) < min_y_oev
    %        min_y_oev = output_vertices_in_screen(2, i);        
    %    elseif output_vertices_in_screen(2, i) > max_y_oev
    %        max_y_oev = output_vertices_in_screen(2, i);        
    %   end
    %end

    % diff_x_oev = screen_max_x - screen_min_x;
    % diff_y_oev = max_y_oev - min_y_oev;

    % minXScreen = screen_min_x;
    % maxXScreen = screen_max_x;
    % minYScreen = min_y_oev;
    % maxYScreen = max_y_oev;
